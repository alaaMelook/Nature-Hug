...
export default function AllOrdersPage() {
    ...
    const handleExport = async (status: string) => {
        ...
        const mapped = (orders.filter((o: any) => o.order_status === status) ?? []).map((o: any) => [
            o.id,
            o.status,
            new Date(o.created_at).toLocaleString('en-GB', { timeZone: 'Africa/Cairo', hour12: true }),
            o.customers?.name ?? "-",
            o.customers?.phone ?? "-",
                    ...
                    {(orders?.length ?? 0) > 0 ? (
                        orders.map((order) => (
                            ...
                            >
                                ...
                                <td className="px-4 py-2 border-b">{new Date(order.order_date).toLocaleString('en-GB', { timeZone: 'Africa/Cairo', hour12: true }).split(',').map((info, ind) =>
