...
import { Save, ArrowLeft, Check, Search, Users, Calendar, Store } from "lucide-react";
...
export default function EditPromoCodeForm({ promoCode, products, customers = [] }: EditPromoCodeFormProps) {
    ...
    const [bogoDiscountPct, setBogoDiscountPct] = useState<number>(promoCode.bogo_discount_percentage ?? 100);
    ...
    const [bazaarOnly, setBazaarOnly] = useState<boolean>(promoCode.bazaar_only || false);
    const [bazaarIds, setBazaarIds] = useState<number[]>(
        promoCode.bazaar_ids?.length ? promoCode.bazaar_ids : (promoCode.bazaar_id ? [promoCode.bazaar_id] : [])
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await updatePromoCodeAction({
                ...
                bogo_discount_percentage: isBogo ? bogoDiscountPct : 100,
                all_cart: allCart,
                            ...
                            <div className="space-y-4 bg-purple-50 p-4 rounded-lg border border-purple-100">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-purple-900 mb-1">
                                        {i18n.language === 'ar' ? 'نسبة الخصم علي المنتج الأرخص' : 'Discount % on cheapest item(s)'}
                                    </label>
                                    <div className="relative">
                                        <input
                                            type="number"
                                            ...
                                            onChange={(e) => setBogoDiscountPct(Math.min(100, Math.max(1, parseInt(e.target.value) || 100)))}
                                            className="w-full px-3 py-2 border border-purple-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none pr-8"
                                        />
                                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-purple-500 font-medium">%</span>
                                    </div>
                                    <p className="text-xs text-purple-600 mt-1">
                                        {i18n.language === 'ar' ? '100% = مجاني بالكامل، 50% = نص السعر' : '100% = fully free, 50% = half price'}
                                    </p>
                                </div>
                            </div>
                        ) : (
