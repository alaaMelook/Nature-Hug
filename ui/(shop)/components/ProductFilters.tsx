...
export default function ProductFilters({ onFilterChange }: { onFilterChange: (filters: any) => void }) {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        onFilterChange({ [name]: isCheckbox ? checked : value });
    };
    let { data: categories } = useCategories();

    return (
        <div className="bg-primary-10 p-6 rounded-lg shadow-lg h-3/5 w-85 fixed ">
                ...
                <div>
                    ...
                    >
                        ...
                        {categories?.map(category => (
                            <option key={category.id} value={category.id}> {langStore.get() === 'en' ? category.name_en : (category.name_ar ?? category.name_en)}</option>
                        ))}
                    </select>
                </div>
                <div>
