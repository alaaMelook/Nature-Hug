    ...
    <nav className="bg-white pl-10 pr-10 sticky top-0 z-10 flex items-center justify-between shadow-md">
      ...
      <div className="hidden md:flex items-center space-x-4">
        ...
        >
          ...
          Cart
          {count > 0 && (
            <span className="absolute -right-2 -top-2 text-xs min-w-5 h-5 px-1 rounded-full text-white bg-primary-800 flex items-center justify-center">
              {count}
            </span>
          )}
        </Link>
      ...
      {/* ---- Mobile Menu ---- */}
      {isOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full bg-white shadow-md flex flex-col items-center py-4 space-y-2 z-50">
          {navigationItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
          ...
          >
            ...
            Cart
            {count > 0 && (
              <span className="text-xs w-5 h-5 rounded-full ml-2 bg-primary-800 text-white flex items-center justify-center">
                {count}
              </span>
            )}
          </Link>
